...
class PostService {
    ...
    public function __construct() {
        ...
        add_action('admin_print_scripts', function() {
            ...
            if($screen && $screen->base == 'post' && $screen->post_type == Environment::postType()) {
                ...
                $savingSectionStatesDisabled = SettingService::isSavingSectionStateDisabled() ? 'true' : 'false';
                echo "
                    <script type='text/javascript'>
                        ...
                        var wpccDisableSaveSectionStates = {$savingSectionStatesDisabled};
                    </script>
                ";
            }
        });
     ...
     * @since 1.9.0
     * @since 1.13.0 Make public
     */
    public function getTransformableFieldsOptions(SettingsImpl $postSettings): array {
     ...
     * @since 1.8.0
     */
    private function getOptionsBoxConfigs($postSettings): array {
        /** @var string[] $postContentRegularButtonNames */
        $postContentRegularButtonNames = array_filter(array_map(function(ShortCodeButton $button): ?string {
            $code = $button->getCode();
            return $code !== ShortCodeName::WCC_MAIN_CONTENT
                ? $code
                : null;
        ...
        $configs = [
            ...
            SettingKey::POST_TITLE_SELECTORS => OptionsBoxConfiguration::init()
                ->addTabOption(OptionsBoxTab::TEMPLATES, TemplatesTabOptions::ALLOWED_REGULAR_SHORT_CODES, [
                    ShortCodeName::WCC_ITEM,
                ])
                ->get(),

            // Post excerpt selectors
            SettingKey::POST_EXCERPT_SELECTORS => OptionsBoxConfiguration::init()
                ->addTabOption(OptionsBoxTab::TEMPLATES, TemplatesTabOptions::ALLOWED_REGULAR_SHORT_CODES, [
                    ShortCodeName::WCC_ITEM,
                    ShortCodeName::WCC_MAIN_TITLE,
                ])
                ->get(),

            // Post content selectors
            SettingKey::POST_CONTENT_SELECTORS => OptionsBoxConfiguration::init()
                ->addTabOption(OptionsBoxTab::TEMPLATES, TemplatesTabOptions::ALLOWED_REGULAR_SHORT_CODES, array_merge(
                    [ShortCodeName::WCC_ITEM],
                    $postContentRegularButtonNames,
                ))
                ->get(),
        ...
        return array_merge($configs, PostDetailsService::getInstance()->getOptionsBoxConfigs($postSettings));
    }
    ...
    public function getEditorButtonsMain(): array {
     ...
     *
     * @return string[] An array of short code names, without square brackets
     */
    public function getPredefinedShortCodes(): array {
