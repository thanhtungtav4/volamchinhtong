...
function volam_setup() {
	/*
		...
		* If you're building a theme based on volam, use a find and replace
		* to change 'volam' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'volam', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'volam' ),
		)
	);
 ...
 */
function volam_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'volam_content_width', 640 );
}
add_action( 'after_setup_theme', 'volam_content_width', 0 );
